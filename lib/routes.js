...
async function handleApiRoutes(req, res, pathname) {
  ...
  if (req.method === "POST" && adminListingApprovalMatch) {
    ...
    createNotification(
      ...
      "listing-approved",
      "تم اعتماد الإعلان",
      `تمت مراجعة إعلانك "${listing.title}" وأصبح ظاهرًا الآن داخل المنصة.`,
      listingId
    );

    sendJson(res, 200, {
      message: "تم اعتماد الإعلان وأصبح ظاهرًا للزوار.",
      listing: getAdminListingById(listingId, admin),
      listings: getAdminListings(admin),
      stats: getAdminStats(),
    });
  ...
  if (req.method === "POST" && (pathname === "/api/listings" || pathname === "/api/listings/create")) {
    ...
    try {
      ...
      sendJson(res, 201, {
        message: isFeatured
          ? "تم إرسال الإعلان وطلب تمييزه، وهو الآن بانتظار مراجعة الإدارة."
          : "تم إرسال الإعلان بنجاح وهو الآن بانتظار مراجعة الإدارة.",
